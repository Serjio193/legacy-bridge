...
echo ========================================
echo.
echo This will flash the Recovery factory image to 0x300000
echo.
...
echo Command: python -m esptool write_flash 0x300000 "%RECOVERY%"
echo.

REM Find usable Python with esptool
for %%p in (py python python.exe) do (
    echo Trying %%p...
    %%p -m esptool --chip esp32c3 --port %PORT% --baud 921600 --before default_reset --after hard_reset write_flash -z --flash_mode dio --flash_freq 80m 0x300000 "%RECOVERY%"
