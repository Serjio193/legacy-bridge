...
It is an embedded ESP32-based modification for soldering stations (Aixun T420D and similar) that adds automatic extractor control based on soldering iron and hot air operation.
...
After buying the Aixun ES02 fume extractor, I found that it did not work with my T420D soldering station because the station had no Wi-Fi module. At that time, I did not know that Wi-Fi versions of this station existed. Like many users, I thought Aixun T420D came in only one variant.
...
First, Aixun ES02 Bluetooth control was researched. After command analysis, it became clear that the extractor can be controlled directly.
...
- Power-off the station/device
- Remove both irons from holders (Handle 1 + Handle 2 are raised)
- Power on the device
- Device starts in `Recovery` mode

If the `System` image is damaged or missing, the device automatically boots to `Recovery` for restore.
...
Після покупки димовловлювача Aixun ES02 з'ясувалося, що він не працює з моєю паяльною станцією Aixun T420D, оскільки в ній відсутній Wi-Fi модуль. На той момент я не знав, що існують версії цієї станції з Wi-Fi — як і багато хто, я був упевнений, що Aixun T420D буває лише в одному варіанті.

Також виявилося, що димовловлювач не працює і з феном Aixun H312.
...
Також було досліджено фен (Aixun H312) через усі доступні інтерфейси: USB, Wi-Fi та Bluetooth. Підключення вдалося встановити через кожен з них, однак у проєкті використовується лише бездротова інтеграція як найбільш практична.
...
Також варто зазначити, що Aixun пропонує окремий пристрій — Aixun BS08 AI Voice Center Control, призначений для керування обладнанням через голосові команди.
...
- Вимкніть станцію/пристрій
- Зніміть обидва паяльники з підставок (Handle 1 + Handle 2 у стані "піднято")
- Увімкніть пристрій
- Пристрій запуститься в режимі `Recovery`

Якщо образ `System` пошкоджений або відсутній, пристрій автоматично переходить у `Recovery` для відновлення.
...
После покупки дымоуловителя Aixun ES02 выяснилось, что он не работает с моей паяльной станцией Aixun T420D, так как в ней отсутствует Wi-Fi модуль. На тот момент я не знал, что существуют версии этой станции с Wi-Fi — как и многие, я был уверен, что T420D бывает только в одном варианте.
...
Также был исследован фен (Aixun H312) по всем доступным интерфейсам: USB, Wi-Fi и Bluetooth. Подключение удалось установить по каждому из них, однако в проекте используется только беспроводная интеграция как наиболее практичная.
...
*Фото 4: пример установки модуля внутри Aixun T420D*
...
- Выключите станцию/устройство
- Снимите обе ручки с подставок (Handle 1 + Handle 2 в состоянии "поднято")
- Включите устройство
- Устройство загрузится в режиме `Recovery`

Если образ `System` повреждён или отсутствует, устройство автоматически загрузится в `Recovery` для восстановления.
